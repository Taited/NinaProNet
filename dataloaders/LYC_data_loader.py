...
from data_process.data_transform import *
...
class LYC_dataset(Dataset):
    ...
    def __init__(self, config, stage):
        ...
        self.train_patch_size = self.config['train_patch_size']
        self.test_patch_size = self.config['test_patch_size']
        self.classnum = int(self.config['class_num'])
        self.img_name = self.config['image_name']
        ...
        self.load_feature = self.config['load_feature']
        ...
        self.stride_xy = self.config['stride_xy']
        self.stride_z = self.config['stride_z']
    ...
    def get_subimage_batch(self):
        ...
        if multichannel:
            ...
            for i in range(self.batchsize):
                ...
                shapemin = [random.randint(0, originaldata.shape[ii+1]-self.train_patch_size[ii]) for ii in range(3)]
                subdata = originaldata[:, shapemin[0]:shapemin[0]+self.train_patch_size[0], shapemin[1]:shapemin[1]+self.train_patch_size[1],
                                    shapemin[2]:shapemin[2]+self.train_patch_size[2]]
                sublabel = originallabel[shapemin[0]:shapemin[0]+self.train_patch_size[0], shapemin[1]:shapemin[1]+self.train_patch_size[1],
                                shapemin[2]:shapemin[2]+self.train_patch_size[2]]
            ...
            for i in range(self.batchsize):
                ...
                shapemin = [random.randint(0, originaldata.shape[ii]-self.train_patch_size[ii]) for ii in range(3)]
                subdata = originaldata[shapemin[0]:shapemin[0]+self.train_patch_size[0], shapemin[1]:shapemin[1]+self.train_patch_size[1],
                                shapemin[2]:shapemin[2]+self.train_patch_size[2]]
                sublabel = originallabel[shapemin[0]:shapemin[0]+self.train_patch_size[0], shapemin[1]:shapemin[1]+self.train_patch_size[1],
                                shapemin[2]:shapemin[2]+self.train_patch_size[2]]
    ...
    def get_list_overlap_img(self, patient_order):
        ...
        if multichannel:
            ...
            img_number = int(math.ceil((originaldata.shape[0]-self.test_patch_size[0])/self.stride_z))  # 在z轴上切成的块数,若为小数会向上取整
            for i in range(img_number - 1):
                subdata = originaldata[i * self.stride_z:i*self.stride_z+self.test_patch_size[0]][np.newaxis, :][
                          np.newaxis, :]
                subdata = torch.from_numpy(subdata).float()
                databatch.append(subdata)
            subdata = originaldata[-self.test_patch_size[0]::][np.newaxis, :][np.newaxis, :]
            subdata = torch.from_numpy(subdata).float()
            databatch.append(subdata)
        else:
