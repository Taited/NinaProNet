...
class Unet_Separate_3_dis2(Module):
    """
    ...
    def __init__(self, inc=1, n_classes=5, base_chns=12, droprate=0, norm='in', depth = False, dilation=1, separate_direction='axial',output_feature=False):
        super(Unet_Separate_3_dis2, self).__init__()
        self.model_name = "seg"
        ...
        self.conv8_dis_1 = TriResSeparateConv3D(4 * base_chns, 2 * base_chns, norm=norm, depth=depth, pad='same',
                                                dilat=dilation, separate_direction=separate_direction)
        self.conv8_dis_2 = TriResSeparateConv3D(2 * base_chns, 2 * base_chns, norm=norm, depth=depth, pad='same',
                                                dilat=dilation, separate_direction=separate_direction)
        self.conv8_dis_3 = nn.Sequential(
            nn.Dropout3d(p=0.1),
    ...
    def forward(self, x):
        ...
        dis = self.conv8_dis_1(conv8)
        dis = self.conv8_dis_2(dis)
        dis = self.conv8_dis_3(dis)
        dis = t.sigmoid(dis)
