...
class DS_Unet_Separate_5(Module):
    """
    ...
    def __init__(self, inc=1, n_classes=5, base_chns=12, droprate=0, norm='in', depth = False, dilation=1):
        super(DS_Unet_Separate_5, self).__init__()
        self.model_name = "seg"
        ...
        self.classification0 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='trilinear'),
            nn.ReLU(),
            nn.Dropout3d(p=0.1),
            TriResSeparateConv3D(8 * base_chns, 4 * base_chns, norm=norm, depth=depth, pad='same', dilat=dilation),
            nn.Upsample(scale_factor=2, mode='trilinear'),
            nn.ReLU(),
            nn.Dropout3d(p=0.1),
    ...
    def forward(self, x, mode):
        ...
        predictt0 = F.softmax(self.classification0(out), dim=1)
        ...
        predict1 = F.softmax(self.classification1(out), dim=1)
        out = self.conv7_1(out)
        out = self.conv7_2(out)

        out = self.upsample(out)  # 1/2
        out = t.cat((out, conv1), 1)
        predict2 = F.softmax(self.classification2(out), dim=1)
        out = self.conv8_1(out)
        out = self.conv8_2(out)

        out = self.classification3(out)
        predict3 = F.softmax(out, dim=1)

        if mode =='train':
            return [predictt0, predict1, predict2, predict3]
        else:
            return predict3
