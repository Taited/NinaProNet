...
def load_nifty_volume_as_array(filename, transpose=True, return_spacing=False, respacing=False,target_spacing=1,
                                 assigned_zoom_factor=False,mode='image', order=3):
    """
    ...
    if transpose:
        ...
        spacing.reverse()
    if respacing:
        if assigned_zoom_factor:
            zoomfactor = assigned_zoom_factor
        else:
        ...
        if mode == 'label':
            data = resize_Multi_label_to_given_shape(data,
                                                     zoom_factor=zoomfactor,class_number=np.max(data)+1, order=order)
        elif mode == 'image':
            data = ndimage.zoom(data, zoom=zoomfactor, order=order)
        else:
            raise ValueError('Please choose the correct interpolation mode!')
        spacing = target_spacing
    if return_spacing:
        if respacing:
            return data, spacing, zoomfactor
        else:
            return data, spacing
    else:
        return data
...
def resize_Multi_label_to_given_shape(volume, zoom_factor, class_number, order=3):
    ...
    volum_one_reshape = [ndimage.zoom(volume_one[i + 1], zoom_factor, order=order) for i in
                         range(class_number-1)]
    output = np.zeros_like(volum_one_reshape[0])
    for i in range(class_number - 1):
        output[np.where(np.rint(volum_one_reshape[i])==1)] = (i + 1)
    return output
...
def convert_to_one_hot(volume, class_number):
    ...
    :return:
    '''
    hot_img = np.zeros([class_number] + list(volume.shape))
    for i in range(class_number):
        hot_img[i][np.where(volume == i)] = 1
    return hot_img
